...
rules:
  ...
  - allow:
      role: clerk
      objects: "^invoice@items"

  - deny:
      role: clerk
      objects: "^invoice@total"
```
...
Combining roles and special keys:
...
- a `.sig` file is created

On read:
- signatures are verified if present
- policy rules are applied per object
...
This system provides cryptographic integrity, fine-grained authorization, deterministic validation, and compatibility with distributed offline-first workflows. It does not provide global consensus, total ordering, or automatic convergence across nodes with different policies.
